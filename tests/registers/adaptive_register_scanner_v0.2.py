#!/usr/bin/env python3
r"""
EL30V2 Adaptive Register Scanner v0.2
...
C:\Python310\python.exe tests\registers\adaptive_register_scanner_v0.2.py
"""
...
# Console status frequency. Readable hits, health events, pass changes, and
# genuinely unexpected errors are printed immediately. Expected Modbus
# exception responses from unreadable addresses are intentionally quiet.
...
class Scanner:
    ...
    def _recount(self) -> None:
        ...
        self.unreadable_count = sum(
            1 for result in self.results.values() if result.status == "UNREADABLE"
        )
    ...
    async def raw_read_one(self, address: int) -> Result:
        ...
        try:
            ...
            # During reconnaissance, an FC03 Modbus exception response is an
            # expected negative result: the probed address is not readable.
            # DeviceSession currently surfaces these as ValueError because the
            # exception frame contains no register data bytes. Keep the full
            # diagnostic detail in the state journal, but do not treat it as a
            # scanner fault or flood the console with it.
            if (
                isinstance(exc, ValueError)
                and "Expected 2 data bytes for 1 registers, got 0" in str(exc)
                and any("ResponseHex=01 83" in item for item in diagnostics)
            ):
                return Result("UNREADABLE", None, detail)
        ...
        *,
        ...
        if count_as_scan_probe:
            ...
            if result.status == "READABLE":
                ...
                # Expected negative discovery result. It is persisted in the
                # state journal but deliberately omitted from console output.
    ...
    def print_progress(self, current: int, source: str) -> None:
        ...
        print(
            ...
            f"unreadable={self.unreadable_count} "
    ...
    def write_map(self) -> None:
        ...
        with MAP_FILE.open("w", encoding="utf-8") as fh:
            ...
            fh.write(f"Unreadable: {self.unreadable_count}\n")
            ...
            fh.write("-----------------------------\n")
            fh.write(
                "UNREADABLE means the device returned an expected Modbus "
                "exception response for that address.\n"
            )
            fh.write(
    ...
    async def run(self) -> int:
        ...
        print()
        print("EL30V2 Adaptive Register Scanner v0.2")
        print("-------------------------------------")
